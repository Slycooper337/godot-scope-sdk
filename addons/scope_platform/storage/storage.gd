...
func upload(file_path: String, metadata: Dictionary = {}) -> ScopeResponse:
	...
	if result.success:
		result.data = ScopeStorageFile.from_json(result.data)
	return result
...
func download(file_id: int, destination_path: String = "") -> ScopeResponse:
	...
	if file == null:
		return ScopeResponse.fail(0, "Could not open download destination")
	file.store_buffer(result.data)
	result.data = destination_path
	return result

func delete(file_id: int) -> ScopeResponse:
	return await _api.request_delete("/storage/files/%d" % file_id)
