...
static func from_json(value: Variant) -> ScopeWizardsWagerBet:
	...
	if value is Dictionary:
		...
		bet.choice = _first_string(data, ["choice", "bet_on", "selection", "prediction", "side", "bet_choice"])
		...
		bet.resolves_at = _first_string(data, ["resolves_at", "resolve_at", "scheduled_for", "job_run_at", "next_run_at", "job_at", "flip_at", "expires_at"])
		bet.job_at = bet.resolves_at
		bet.server_time = _first_string(data, ["server_time", "current_time", "now"])
	return bet
