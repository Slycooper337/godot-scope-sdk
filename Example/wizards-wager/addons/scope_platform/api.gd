...
func request_multipart_file(endpoint: String, file_path: String, fields: Dictionary = {}) -> ScopeResponse:
	...
	body.append_array(("\r\n--%s--\r\n" % boundary).to_utf8_buffer())
	return await _request_raw(HTTPClient.METHOD_POST, endpoint, "multipart/form-data; boundary=%s" % boundary, body, true)


func request_bytes(endpoint: String) -> ScopeResponse:
	return await _request_raw(HTTPClient.METHOD_GET, endpoint, "", PackedByteArray(), false)
...
func _request_unlocked(
	...
	if ScopeConfig.debug_logging():
		...
		print("[Scope] application_id_present=%s public_key_present=%s" % [
			not ScopeConfig.application_id().is_empty(),
			not ScopeConfig.public_key().is_empty()
		])
		print("[Scope] authorization_present=%s" % _session.is_logged_in())
...
func _request_raw_unlocked(method: HTTPClient.Method, endpoint: String, content_type: String, body: PackedByteArray, parse_json: bool) -> ScopeResponse:
	...
	if response_code >= 200 and response_code < 300:
		if parse_json:
			...
			return ScopeResponse.ok(response_code, JSON.parse_string(text) if not text.is_empty() else null)
